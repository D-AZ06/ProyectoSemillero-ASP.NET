...
namespace ProyectoSemillero_ASP.NET.Controllers
{
    public class UsuariosController : Controller
    {
        ...
        [HttpGet]
        public ActionResult Agregar(int? idSemillero = null)
        {
            if (Session["Rol"] == null) return RedirectToAction("IniciarSesion", "Home");
            string rolUsuario = Session["Rol"].ToString();
            ...
            if (idSemillero.HasValue)
            {
                nuevoUsuario.IdSemillero = idSemillero.Value;
            }

            var listaIds = coleccionUsuarios.Find(new BsonDocument())
            ...
            foreach (int id in listaIds)
            {
                ...
                if (idStr.StartsWith("10") && idStr.Length >= 3)
                {
                    string consecutivoStr = idStr.Substring(2);
                    if (int.TryParse(consecutivoStr, out int consecutivo))
                    {
            ...
            nuevoUsuario.IdUsuario = maxConsecutivo >= 0
                ? int.Parse("10" + (maxConsecutivo + 1))
                : 100;
        ...
        public ActionResult Agregar(DatosUsuario nuevoUsuario)
        {
            ...
            // ==========================================
            // REGLA: GENERACIÓN DE ID (Patrón "10" + Consecutivo)
            // ==========================================
            var listaIds = coleccionUsuarios.Find(new BsonDocument())
                .Project(u => u.IdUsuario)
                .ToList();
